...
#define _HOOK_DISPATCH(F, ...) \
    ...
    uintptr_t h_ret = (uintptr_t) ((F)hook_##F.addr)(__VA_ARGS__); \
    hijack_resume(&hook_##F) 
#define _HOOK_DISPATCH2(F, ...) \
    hijack_pause(&hook_##F); \
    uintptr_t h_ret2 = (uintptr_t) ((F)hook_##F.addr)(__VA_ARGS__); \
    hijack_resume(&hook_##F) 
...
STATIC int dyn_sym_tab_init(void *h, dyn_fn_t *fn_tab, uint32_t cnt){
    for(int i = 0; i < cnt; i++)
    {
...
void remove_hooks(hook_entry_t *hooks, uint32_t cnt){
    for(int i = 0; i < cnt; i++)
    {
