...
#define cacheflush(from, size)   __clear_cache((void*)from, (void*)((uintptr_t)from+size))

void hijack_start(sym_hook_t *sa, void *target, void *_new) {
    uint8_t o_code[HIJACK_SIZE], n_code[HIJACK_SIZE];
    ...
    if ( (uintptr_t)target % 4 == 0 )
    {
        ...
        *(uintptr_t *)&n_code[4] = (uintptr_t)_new;
